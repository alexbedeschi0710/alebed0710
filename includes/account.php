 ...
 *
 * Sezioni:
 *  1. Foto profilo (upload da galleria/fotocamera, meta pz_avatar)
 *  2. Livello di gioco compatto (riusa pz_ur_levels_html)
 *  3. Dati personali: Nome, Cognome, Email, Telefono
 */
...
/* ============================================================
 *  HELPER — URL avatar
 *  Prima legge pz_avatar (caricato dall'utente),
 *  poi fallback su get_avatar_url (Gravatar/plugin avatar)
 * ============================================================ */
function pz_get_user_avatar_url($user_id = null) {
    ...
    return get_avatar_url($user_id, ['size' => 128, 'default' => 'mm']);
}
...
add_shortcode('pz_account', function() {
    ...
    .pza-level-btn.active .pza-level-check {
        background: #1FB856 !important; border-color: #1FB856 !important;
    }
    ...
    .pza-level-save {
        ...
        padding: 11px !important; cursor: pointer !important;
    ...
    .pza-input[disabled] { opacity: .5 !important; cursor: not-allowed !important; }
    ...
    <div id="pzAccount">
        ...
        <div class="pza-section">
            <div class="pza-avatar-row">
                <div class="pza-avatar-wrap">
                    <img id="pzaAvatarImg"
                         src="<?php echo esc_url($avatar ?: 'data:image/svg+xml,<svg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 64 64\'><rect fill=\'%23E8F8EE\' width=\'64\' height=\'64\' rx=\'32\'></rect><text x=\'50%\' y=\'54%\' dominant-baseline=\'middle\' text-anchor=\'middle\' fill=\'%231FB856\' font-size=\'28\' font-family=\'Arial\'>' . strtoupper(substr($fname ?: $user->display_name, 0, 1)) . '</text></svg>'); ?>"
        ...
        <div class="pza-section">
            ...
            <div class="pza-field">
                ...
                <input class="pza-input" type="text" id="pzaFname"
                       value="<?php echo esc_attr($fname); ?>" placeholder="Il tuo nome" autocomplete="given-name">
            </div>
            <div class="pza-field">
                <label class="pza-label" for="pzaLname">Cognome</label>
                <input class="pza-input" type="text" id="pzaLname"
                       value="<?php echo esc_attr($lname); ?>" placeholder="Il tuo cognome" autocomplete="family-name">
            </div>
            <div class="pza-field">
                <label class="pza-label" for="pzaEmail">Email</label>
                <input class="pza-input" type="email" id="pzaEmail"
                       value="<?php echo esc_attr($email); ?>" placeholder="La tua email" autocomplete="email">
            </div>
            <div class="pza-field">
                <label class="pza-label" for="pzaPhone">Telefono</label>
                <input class="pza-input" type="tel" id="pzaPhone"
                       value="<?php echo esc_attr($phone); ?>" placeholder="+39 333 000 0000" autocomplete="tel">
            </div>
            ...
            xhr.onload = function() {
                ...
                try {
                    ...
                    if (res.success) {
                        ...
                        toast(res.data || 'Errore upload', false);
                    }
                } catch(err) {
...
add_action('wp_ajax_pz_upload_avatar', function() {
    ...
    if (!in_array($mime, $allowed)) {
        wp_send_json_error('Formato non supportato (usa JPG, PNG, WEBP)');
    }

    // Max 5MB
    if ($_FILES['avatar']['size'] > 5 * 1024 * 1024) {
        wp_send_json_error('Immagine troppo grande (max 5MB)');
    }
...
add_action('wp_ajax_pz_save_profile', function() {
    ...
    // Aggiorna email WP solo se cambiata
